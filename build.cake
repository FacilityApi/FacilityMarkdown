...
var docsAssemblyDirectory = $"src/fsdgenmd/bin/{configuration}";
...
{
	ExecuteCodeGen($@"{File("example/ExampleApi.fsd")} {Directory("example/docs")}/", verify);
}

void ExecuteCodeGen(string args, bool verify)
{
	string exePath = File($"src/fsdgenmd/bin/{configuration}/fsdgenmd.exe");
	if (IsRunningOnUnix())
	{
		args = exePath + " " + args;
		exePath = "mono";
